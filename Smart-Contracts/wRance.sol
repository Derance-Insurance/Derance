...
// File: contracts/IRANCE.sol

pragma solidity 0.5.17;


contract IRANCE is IERC20 {
...
// File: contracts/wRANCE.sol

pragma solidity 0.5.17;

contract wRANCE is ERC20, ERC20Detailed, ERC20Permit {
    using SafeERC20 for ERC20;
    using SafeMath for uint256;

    IRANCE public RANCE;

    modifier notwRANCE(address recipient) {
        require(recipient != address(this), "wRANCE: can not send to self");
        _;
    }

    constructor(IRANCE _RANCE) public ERC20Detailed("Wrapped RANCE", "wRANCE", 18) {
        RANCE = _RANCE;
    }
    ...
    function wrap(uint256 _amount) external {
        require(RANCE.transferFrom(msg.sender, address(this), _amount), "wRANCE: transferFrom failed");
    ...
    function unwrapTo(address _to, uint256 _amount) public notwRANCE(_to) {
        _burn(msg.sender, _amount);
        require(RANCE.transfer(_to, _amount), "wRANCE: transfer failed");
    }
    ...
    {
        if (RANCE.allowance(_owner, address(this)) < _amount) {
            return (false, "insufficient allowance");
        }

        if (RANCE.balanceOf(_owner) < _amount) {
            return (false, "insufficient RANCE balance");
        }

        if (RANCE.isLockedForMV(_owner) > now) {
            return (false, "RANCE balance lockedForMv");
        }

        if (!RANCE.whiteListed(address(this))) {
            return (false, "wRANCE is not whitelisted");
        }
        ...
        returns (bool success, string memory reason)
    {
        if (balanceOf(_owner) < _amount) {
            return (false, "insufficient wRANCE balance");
        }

        if (!RANCE.whiteListed(_recipient)) {
            return (false, "recipient is not whitelisted");
        }

        if (RANCE.isLockedForMV(address(this)) > now) {
            return (false, "wRANCE is lockedForMv");
        }
    ...
    function claimTokens(ERC20 _token, address payable _to, uint256 _balance) external {
        require(_to != address(0), "wRANCE: can not send to zero address");

        if (_token == ERC20(address(RANCE))) {
            uint256 surplusBalance = _token.balanceOf(address(this)).sub(totalSupply());
            require(surplusBalance > 0, "wRANCE: there is no accidentally sent RANCE");
            ...
            uint256 balance = _balance == 0 ? totalBalance : Math.min(totalBalance, _balance);
            require(balance > 0, "wRANCE: trying to send 0 balance");
            _token.safeTransfer(_to, balance);
        }
    }
}    
