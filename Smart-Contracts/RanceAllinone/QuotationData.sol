...
contract QuotationData is Iupgradable {
        ...
        onlyInternal
    {
        ...
        allCovers.push(Cover(_userAddress, _currencyCode,
                _sumAssured, _coverPeriod, expiryDate, _scAddress, premiumRANCE));
        uint cid = allCovers.length.sub(1);
        userCover[_userAddress].push(cid);
        emit CoverDetailsEvent(cid, _scAddress, _sumAssured, expiryDate, premium, premiumRANCE, _currencyCode);
    }
    ...
    function getCoverPremiumRANCE(uint _cid) external view returns(uint _premiumRANCE) {
        _premiumRANCE = allCovers[_cid].premiumRANCE;
    }
    ...
    /// @return premiumRANCE premium in RANCE
