  ...
  <div id="stake-withdraw-summary" v-loading.fullscreen.lock="loading"
    ...
    <el-card :style="{top: top+'px'}">
      ...
      <div style="margin-bottom: 20px;">
        ...
        <el-row>
          <span>{{$toFixed(usedAmount)}} RANCE</span>
          <span style="float: right;">{{$toFixed(maxTotalAmount - usedAmount)}} RANCE</span>
        </el-row>
        <br />
        <el-form label-width="120px" label-position="left">
          ...
          <el-form-item label="Deposit:">
            {{$toFixed(options.deposit)}} RANCE
          </el-form-item>
          <div>
            <highlight>After</highlight>
          </div>
          <el-form-item label="Deposit:">
            {{$toFixed(options.deposit - options.withdraw)}} RANCE
          </el-form-item>
        </el-form>
      </div>
  ...
  methods: {
    ...
    checkContinue(){
      ...
      if(vBN.gt(this.$etherToValue(this.options.maxWithdraw))){
        this.$message.error(`You can withdraw ${this.$etherToValue(this.options.maxWithdraw)} RANCE maximum.`);
        return;
      }
      return true;
    },
