<template>
  <el-card id="stake-withdraw-withdraw" class="box-card">
    ...
    <el-row class="normal-text" style="line-height: 25px;margin-top: 10px;">
      ...
      You can withdraw {{$etherToValue(options.maxWithdraw)}} RANCE maximum.
    </el-row>
    </el-form>
  </el-card>
</template>
  ...
  methods: {
    ...
    validateAmount(rule, value, callback){
      ...
      if(vBN.gt(this.$etherToValue(this.options.maxWithdraw))){
        ...
        callback(new Error(`You can withdraw ${this.$etherToValue(this.options.maxWithdraw)} RANCE maximum.`));
        return;
      }
      callback();
    }
