      ...
      <div style="margin-bottom: 20px;">
        <el-form label-width="120px" label-position="left">
          ...
          <el-form-item label="Pending:">
            {{$toFixed(unstaked)}} RANCE
          </el-form-item>
          <div>
            <highlight>After</highlight>
          </div>
          <el-form-item label="Pending:">
            {{$toFixed(allUnstake)}} RANCE
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    confirm(){
      ...
      if(vBN.lte(0)){
        this.$message.error(`Unstake ${0} RANCE minimum all projects.`);
        return;
      }
      this.$emit("confirm");
    },
    checkContinue(){
      ...
      if(errCount > 0){
        this.error = `Remaining stake ${this.settings.stake.minAmountPerContract} RANCE minumum per project.`;
        return false;
      }
      ...
      if(perError > 0){
        this.error = `Unstake ${this.settings.stake.minAmountPerContract} RANCE minumum per project.`;
        return false;
      }
      return true;
    },
