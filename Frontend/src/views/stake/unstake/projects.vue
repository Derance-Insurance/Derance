  ...
  methods: {
    ...
    validateUnstaking(rule, value, callback, row){
      ...
      if(remainingStaked.gt(0) && remainingStaked.lt(this.settings.stake.minAmountPerContract)){
        this.options.error = `Remaining stake ${this.settings.stake.minAmountPerContract} RANCE minumum per contract.`;
        callback(new Error(`Remaining stake ${this.settings.stake.minAmountPerContract} RANCE minumum per contract.`));
        return;
      }
      // unstaking 最少20
      if(BigNumber(value).gt(0) && BigNumber(value).lt(this.settings.stake.minAmountPerContract)){
        this.options.error = `Unstake ${this.settings.stake.minAmountPerContract} RANCE minumum per contract.`;
        callback(new Error(`Unstake ${this.settings.stake.minAmountPerContract} RANCE minumum per contract.`));
        return;
      }
      callback();
    }
