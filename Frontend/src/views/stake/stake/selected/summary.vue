<template>
  <div id="stake-stake-selected-summary" class="secondary-text">
    <el-row>
      <el-form>
        <el-form-item label="Balance">{{$etherToNumber(member.balance)}} RANCE</el-form-item>
      </el-form>
    </el-row>
    <el-progress :text-inside="true" :stroke-width="25" :percentage="percentage" status="exception"></el-progress>
    <el-row>
      <span>{{usedAmountShow}} RANCE</span>
      <span style="float: right;">{{remainingShow}} RANCE</span>
    </el-row>
    ...
    <el-row class="error" v-if="topupValue > 0">
      ...
      Top up with {{topupValueShow}} RANCE or stake {{perAmountShow}} maximum per contract.
      <br/>
    ...
    <el-row class="error" v-else-if="isInsufficientFunds">
      ...
      <div style="margin-bottom: 10px;text-align: center;">
        <el-button type="primary" plain round size="small" @click="buyRANCE" style="width:80%;">Swap Tokens</el-button>
      </div>
    </el-row>
    <el-row class="error" v-else-if="isMin" style="margin-bottom: 10px;text-align: center;">
      <svg-icon icon-class="circle" class="icon-name error-color"></svg-icon>
      Stake {{settings.stake.minAmountPerContract}} RANCE minumum per contract.<br/>
    </el-row>
  </div>
</template>
