...
export default {
  ...
  computed: {
    ...
    bnbTotalPremium(){
      return BigNumber(this.$etherToValue(this.totalPremiumRANCE)).times(this.options.rate).toFixed(2, 1);
    },
    bnbTotalPremiumUSD(){
  ...
  methods: {
    ...
    async getTotalPremiums(){
      ...
      let totalPremiumRANCE = BigNumber(0);
      for(let i=0; i<length;i++){
        const premiumRANCE = await instance.getCoverPremiumRANCE(i);
        totalPremiumRANCE = totalPremiumRANCE.plus(premiumRANCE.toString());
      }
      this.totalPremiumRANCE = totalPremiumRANCE.toString();
      this.loading = false;
    }
  }
}
