<template>
    <div id="BNBAndRANCE" v-loading.fullscreen.lock="loading"
        :element-loading-text="loadingText">
        <el-form :model="form" :rules="rules" ref="form" :disabled="!member.isMember">
        <el-row>
            <h2 class="main-text">Swap</h2>
            <div class="normal-text">Buy or sell RANCE tokens. RANCE tokens grant you proportional power in Derance.</div>
        </el-row>
    ...
    export default {
      ...
      computed: {
        ...
        showReset(){
            ...
            return this.form.type!='RANCE' || amount.comparedTo(0) > 0;
        },
        balance(){
            try{
              ...
              try{
                ...
                if(this.form.type == "BNB"){
                    ...
                    contract.instance.buyToken({ from: this.$CustomWeb3.account, value: fee }).then(response => {
                      ...
                      this.$message.success("Buy RANCE successfully!");
                      this.$emit("refresh");
                      this.loading = false;
                    }).catch((e) => {
                      console.error(e);
                    ...
                    // 出售RANCE前需要查询资金池允许卖的资金金额，资金池超过2000bnb才能出售
                    ...
                    // 系统里用BNB换RANCE价格显示需要优化下。这个结果需要乘以0.975，合约对反向套利有2.5%的惩罚金
                    const realfee = BigNumber(fee).multipliedBy(this.settings.penaltyRate).integerValue().toString();
                    contract.instance.sellRANCETokens(realfee, { from: this.$CustomWeb3.account }).then(response => {
                      console.info(response, response.toString());
                      this.$message.success("Sell RANCE successfully!");
                      this.$emit("refresh");
                      this.loading = false;
                    }).catch((e) => {
                      console.error(e);
