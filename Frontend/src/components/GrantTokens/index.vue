<template>
  <div id="grantToken" v-loading.fullscreen.lock="loading"
    ...
    <el-card class="box-card" v-for="contAllowance in contractsAllowance" v-if="showGrant(contAllowance)">
      ...
      Make sure sufficient RANCE allowance approved before swap or deposit. Current allowance to
    ...
    <el-dialog :title="'Grant ' + curAllowance.contractName + ' permission'"
      ...
      <el-form :model="form" label-width="150px">
        <el-form-item label="Current allowance:">
          <span class="highlight">{{formatAllowance(curAllowance.curAllowance)}}</span> RANCE
        </el-form-item>
        ...
        <el-form-item label="New allowance:">
          ...
          <span v-else style="font-size:22px;">∞</span> RANCE
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async initContract(){
      this.RANCEToken = await this.getContract(RANCETokenContract);
      console.info("RANCEToken:", this.RANCEToken);
    },
    initEvent(){
